...
public class CustomAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) {
        try {
            ...
            String role = authResult.getAuthorities().iterator().next().getAuthority(); // Assuming you have only one role per user
            String accessToken = JWTUtils.generateToken(user, role); // Modify your JWTUtils to accept the role parameter
            UserService userService = (UserService) SpringApplicationContext.getBean("userServiceImpl");
            UserDto userDto = userService.getUser(user);

            // Create a JSON response
            String jsonResponse = "{\"userId\":\"" + userDto.getUserId() + "\",\"" + "role\":\"" + role + "\",\"" + AppConstants.HEADER_STRING + "\":\"" + AppConstants.TOKEN_PREFIX + accessToken + "\"}";
